	   ...
	   seanceInterface seances = new seanceImp();
		 String date_debut = request.getParameter("date_debut");
			...
			String classe = request.getParameter("id_classe");
			 /******Récupération des champs de formulaire de date debut et Fin**********/
		 	 ...
		 	 //les minutes
 	          ...
 	          /////Conversion des H et Minutes de Form
 	          /***debut formulaire****/
			...
			List<seance> oksalle =seances.verifsalle(salle, jours);
			 ...
			 //si matier n'existe pas
			 else if(taille==0)
			 {
					 //test disponobilite prof pour jour
					 
					   if(tailleseance==0) // si le prof n'a pas des cours à ce jour choisi
					{
						//Il va ajouter sans tester les Heures
						...
						%>
						<h1 style="color:green;" id="test">Ajout de seance réussiteeeee</h1>
						<%
						
					}
					 
						  else /**si prof est occupé pour jour choisi **/
						 {
							 // on va parcourir la liste de okseance
							 int pass=0;
							 seanceInterface sn=new seanceImp();
							 for(int i=0;i<tailleseance;i++)
							 {
								 String debut= okseance.get(i).getDate_debut();
								 String fin=okseance.get(i).getDate_fin();
								 
								 String[] dbtab=debut.split(":");
							 	 String[] dbtabFin=fin.split(":");
							 	 //Heure debut de table
							 	  double H=Double.parseDouble(dbtab[0]);
							 	 double mdbut=Double.parseDouble(dbtab[1]);
							 	 double mdbuts=mdbut/100;
							 	 double totdebut=mdbuts+H;
							 	//Heure fin de table
							 	  double HFin=Double.parseDouble(dbtabFin[0]);
							 	 double mdbutFin=Double.parseDouble(dbtabFin[1]);
							 	 double mdbutsFin=mdbutFin/100;
							 	 double totFin=mdbutsFin+HFin;
							 	
							 	//tester la disponoibilité de prof
								if(dbfom<=totdebut && dbFfom>=totdebut)
								{
									pass=1;
									%>
						<h1 style="color:red;" id="test">Proffesseur occupé entre <%=totdebut %> et <%=totFin %></h1>
						<%
						 }
								else if(dbfom<=totFin && dbFfom>=totFin) 
								 {
									pass=1;
								 %>
									<h1 style="color:red;" id="test">Proffesseur occupé entre <%=totdebut %> et <%=totFin %></h1>
						 ...
						 }/**Fin si prof est occupé pour jour choisi **/
					 
					
					 
		 	} //fin matier n'existe pas
