...
<div class="wrapper">
  ...
  String prof=name+" "+preno;
	 ...
	 if(notes.getId()==id_update) 
	 {
		 nom=notes.getMat();
		 date=notes.getExamen_date();
		 debut=notes.getExamen_heure_debut();
		 fin=notes.getExamen_heure_fin();
		 not=notes.getExamen_note();
		 matiere=notes.getMat();
		 salle=notes.getId_salle();
		 etudiant=notes.getEtudiant();
		 classe=notes.getClasse();
		 coefficient=notes.getExamen_coefficient();
		 type=notes.getExamen_type();
	 }
  }
...
<form method="post" action="../notes" id="form">
 <%
   ...
   <label style="position: relative; top: 0px">Matiere</label>
    ...
    <% 
				...
				if(classes!=null)
				 ...
				 {%>
    <option<%if(matiere.equalsIgnoreCase(mt.getMat_nom() )){ %> selected="selected"<%} %> ><%=mt.getMat_nom() %></option>
  <%
				 }
					}
					else
  ...
  <input type="hidden" name="id" value="<%=id %>">
   <select name="type">
   <option  <%if(type.equalsIgnoreCase("orale" )){ %> selected="selected"<%} %>>orale</option>
    <option  <%if(type.equalsIgnoreCase("Ds" )){ %> selected="selected"<%} %>>Ds</option>
    <option <%if(type.equalsIgnoreCase("td" )){ %> selected="selected"<%} %> >TD</option>
    <option <%if(type.equalsIgnoreCase("tp" )){ %> selected="selected"<%} %>>TP</option>
    <option <%if(type.equalsIgnoreCase("examen" )){ %> selected="selected"<%} %>>Examen</option>
    </select>
    <span class="highlight"></span><span class="bar"></span>
  ...
  <div class="group">
    <input type="time" id="db" name="examen_heure_debut" value="<%=debut %>"><span class="highlight"></span><span class="bar"></span>
  
  </div>
  <div class="group">
    <input type="time" id="fin" name="examen_heure_fin" value="<%=fin %>"><span class="highlight"></span><span class="bar"></span>
  ...
  <div class="group">
    <input type="text" name="salle" value="<%=salle %>" placeholder="salle"><span class="highlight"></span><span class="bar"></span>
  
  </div>
  <div class="group">
    <input type="text" id="coefficient" name="coefficient" value="<%=coefficient %>"placeholder="coeficient"><span class="highlight"></span><span class="bar"></span>
  ...
  %>
   ...
   <div class="group">
    <input type="text" id="note" name="note" value="<%=not %>" placeholder="note"><span class="highlight"></span><span class="bar"></span>
