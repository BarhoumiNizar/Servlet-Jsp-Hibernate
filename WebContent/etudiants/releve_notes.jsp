...
%>
   <h1>Relevé de notes <u><b></b></u> </h1>
  
</hgroup>

<%
				//pour parcourire le resultat users
				payementInterface payer=new payementImp();
                 int x=payer.verifPayement(matricule).size();
                 if(x!=0)
             	{
                 List<Payement> ok = payer.verifPayement(matricule);
                 String etat=ok.get(0).getPy_tranche();
                 
                 %>
                 <h1><% %></h1>
                 
						<% if(etat.equalsIgnoreCase("non_valide")) {%>
								<h1><font color=red>Payement non validé</font></h1>
						<% } %>
					 ...
					 <% if(etat.equalsIgnoreCase("valide")) {%>
					  <h1 style="padding-left: 50px"><font color="green"></font></h1>
					   <%
      Object etudiant=session.getAttribute("matricule");
    	   String etudiants=etudiant.toString();
...
<% 
				...
				for( Examen notes:note.getALLNotes())
				{
				  if(notes.getEtudiant().equalsIgnoreCase(etudiants))
				  {
				%>
				<tr height="50">
				       ...
				       <td align="center"><%=notes.getExamen_note()%></td>
				        <td align="center"><%=notes.getExamen_coefficient()%></td>
				...
				<% }} %>
					 ...
					 { %>
					 <h1><font color=red>Votre Payement n'est pas enregistré</font></h1>
					 <% } %>
